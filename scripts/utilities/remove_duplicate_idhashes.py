def remove_duplicate_idhashes(user_data, idhash_col):
    ...
    user_transaction_hashes = (
        tmp_data[idhash_col].explode().rename(duplicate_idhash_col)
    )
    # identify the duplicate idhashes
    duplicated_user_transaction_hashes = (
        user_transaction_hashes[user_transaction_hashes.duplicated()]
        .reset_index()
        .groupby(by="index")
        .agg({duplicate_idhash_col: list})
    )
    ...
    tmp_data[idhash_col] = tmp_data.apply(
        lambda s: s[idhash_col]
        if not s[duplicate_idhash_col] == s[duplicate_idhash_col]
        else [v for v in s[idhash_col] if v not in s[duplicate_idhash_col]],
        axis=1,
    )
