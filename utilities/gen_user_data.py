...
def gen_user_data(user_agg_data, user_obj, device_obj, card_obj, ip_obj, transaction_obj, application_obj):
    ...
    user_data['registration_dates'] = user_data['uid'].replace(user_obj.user_ids_dates_dict)
    ...
    user_data['userid'] = user_data['registration_dates'].dt.year.astype(str) + user_data['registration_country_code'].astype(str) + user_data['uid'].astype(str).str[-9:]
