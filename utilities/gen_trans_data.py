...
def gen_trans_data(user_data, device_obj, card_obj, ip_obj, transaction_obj, application_obj):
    ...
    trans_data['card_country_code'] = trans_data['card_hash'].replace(card_obj.card_hashes_country_codes)
    trans_data['ip_hash'] = trans_data['ip_hash'].apply(lambda x: np.random.choice(x, size = 1)[0])
    trans_data['ip_country_code'] = trans_data['ip_hash'].replace(ip_obj.ip_hashes_country_codes)
    ...
    trans_data['userid'] = trans_data['transaction_date'].dt.year.astype(str) + trans_data['registration_country_code'].astype(str) + trans_data['uid'].astype(str).str[-9:]
